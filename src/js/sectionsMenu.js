...
export default function SectionsMenu() {
    ...
    elements.forEach(element => {
        ...
        function setActiveItem() {
            ...
            const newActiveItem = items.find(element => element.link.classList.contains('active-section'));

            if (newActiveItem) {
                const scrollParent = newActiveItem.link.closest('.sticky-nav__links');
                gsap.to(scrollParent, { duration: 0.3, ease: 'power1.out', scrollTo: { x: newActiveItem.link, offsetX: contentPadding }, clearProps: 'all' });
            }
